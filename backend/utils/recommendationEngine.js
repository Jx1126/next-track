...
async function generateRecommendation(tracks, algorithmType = "hybrid") {
  ...
  const selectedAlgorithm =
    algorithmMap[algorithmType] || ALGORITHM_TYPES.HYBRID;
  ...
  try {
    ...
    const artists = [
      ...new Set(
        tracks.map((track) => track.artist).filter((artist) => artist)
      ),
    ];
    const allTags = tracks
    ...
    externalTracks = await searchMusicBrainzTracks(
      searchQuery,
      RECOMMENDATION_CONFIG.MAX_EXTERNAL_TRACKS
  ...
  const playlistTrackKeys = new Set(
    tracks.map((track) => `${track.title}-${track.artist}`.toLowerCase())
  );
  ...
  try {
    switch (selectedAlgorithm) {
      case ALGORITHM_TYPES.HYBRID:
        recommended_track = recommendByHybrid(
          filteredTracks,
          tracks,
          timestamp
        );
      ...
      case ALGORITHM_TYPES.ARTIST_BASED:
        recommended_track = recommendByArtist(
          filteredTracks,
          tracks,
          timestamp
        );
      ...
      case ALGORITHM_TYPES.TEMPORAL:
        recommended_track = recommendByTemporal(
          filteredTracks,
          tracks,
          timestamp
        );
      ...
      case ALGORITHM_TYPES.LENGTH_BASED:
        recommended_track = recommendByLength(
          filteredTracks,
          tracks,
          timestamp
        );
  ...
  const youtube_link = await generateYoutubeLink(
