...
async function generateYoutubeLink(trackName, artistName) {
  try {
    ...
    const search_results = await youtube_search.GetListByKeyword(search_query, false, 1); // search for the track on YouTube

    // validation: the search results must contain at least one item
    if (!search_results || search_results.length === 0) {
      throw new Error('No YouTube results found for the given track and artist.');
    }
