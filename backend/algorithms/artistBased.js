...
function recommendByArtist(candidateTracks, playlistTracks, timestamp) {
  try {
    ...
    const playlistArtists = playlistTracks
      .map((track) => (track.artist || "").toLowerCase().trim())
    ...
    const artistSimilarityMap = buildArtistSimilarityMap(
      candidateTracks,
      playlistTracks
    );
    ...
    const scoredTracks = candidateTracks.map((track) => {
      ...
      return {
        ...
        explanation:
          matchType === "same_artist"
            ? `Same artist (${track.artist})`
            : matchType === "similar_artist"
            ? `Similar artist to ${uniquePlaylistArtists[0]}`
            : matchType === "related_artist"
            ? `Related to ${uniquePlaylistArtists[0]}`
            : "Artist discovery",
      };
    });
    ...
    return {
      ...
      algorithm_details: {
        ...
        discovery_factor:
          selectedItem.matchType === "discovery"
            ? 1.0
            : selectedItem.matchType === "same_artist"
            ? 0.2
            : selectedItem.matchType === "similar_artist"
            ? 0.7
            : 0.5,
        similarity_breakdown: {
          base_score:
            selectedItem.artistScore > 0.3
              ? selectedItem.artistScore - 0.1
              : selectedItem.artistScore,
          random_factor: 0.1,
        ...
        recommendation_context: {
          ...
          score_range: {
            ...
            lowest:
              scoredTracks[scoredTracks.length - 1].artistScore.toFixed(3),
          },
        },
      },
    };
...
function buildArtistSimilarityMap(candidateTracks, playlistTracks) {
  ...
  const playlistFeatures = playlistTracks.map((track) =>
  ...
  candidateTracks.forEach((track) => {
    ...
    if (!similarityMap.has(artistKey)) {
      ...
      const allPlaylistTags = playlistTracks.map((t) =>
        Array.from(extractTrackFeatures(t).allTags)
      );
      const similarity = comprehensiveJaccardSimilarity(
        trackTagsArr,
        ...allPlaylistTags
      );
      ...
      if (intersection.length > 0) {
        ...
        const artistTagMatches = intersection.filter((tag) =>
        ...
        artistScore =
          (artistTagMatches * 0.2 + tagMatches * 0.8) /
          Math.max(1, intersection.length);
      }
      // use comprehensive similarity score combined with artist discovery logic
      const finalScore = Math.min(
        similarity.weighted * 0.7 + artistScore * 0.3,
        0.9
      ); // cap at 0.9
