  ...
  try {
    ...
    const scoredTracks = candidateTracks.map((track) => {
      try {
        ...
        const trackTags =
          trackFeatures && trackFeatures.allTags ? trackFeatures.allTags : [];
        ...
        console.error(
          `Tag-based: Error scoring track ${track.title || "unknown"}:`,
          error.message
        );
        return {
          track,
    ...
    return {
      ...
      algorithm_details: {
        ...
        track_tags_count: selectedItem.details.trackTags
          ? selectedItem.details.trackTags.length
          : 0,
        ...
        similarity_components: {
          jaccard_base: (selectedItem.details.averageSimilarity * 0.4).toFixed(
            3
          ),
          aggregated_weight: (
            selectedItem.details.aggregatedSimilarity * 0.3
          ).toFixed(3),
          weighted_component: (
            selectedItem.details.weightedSimilarity * 0.2
          ).toFixed(3),
          max_match_bonus: (selectedItem.details.maxSimilarity * 0.1).toFixed(
            3
          ),
        },
  ...
  try {
    ...
    tracks.forEach((track) => {
      try {
        ...
        console.warn(
          `Tag-based: Error extracting tags from track ${
            track.title || "unknown"
          }:`,
          error.message
        );
    ...
    const sortedTagsSet = tagsSet.sort(
