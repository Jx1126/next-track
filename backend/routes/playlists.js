  ...
  try {
    ...
    if (!playlist || !track_id) {
      return res.status(400).json({
        ...
        message: 'Both playlist object and track ID are required to add a track to the playlist'
      });
    }
...
router.delete('/remove', (req, res) => {
  try {
    ...
    if (!playlist || !track_id) {
      return res.status(400).json({
        error: 'Invalid request',
        message: 'Both playlist and track ID are required to remove a track from the playlist'
      });
    }
    ...
    const updated_tracks = playlist.tracks.filter(track => track.id !== track_id);
